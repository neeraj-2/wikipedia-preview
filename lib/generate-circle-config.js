  ...
  for (const [index, value] of fileArray.entries()) {
    data.jobs[`test${index + 1}`] = {
      ...
      steps: [
        ...
        {
          run: {
            name: `Running cypress tests ${index + 1} firefox`,
            requires:
             `cypress/install` ,
            executor: `cypress/browsers-chrome73-ff68`,
            command: `$(npm bin)/cypress run --spec cypress/integration/${value} --browser chrome`,
            group: `firefox`,
            browser: `firefox`
          },
        },
        {
            run: {
              name: `Running cypress tests ${index + 1} chrome`,
              requires:
               `cypress/install`,
              executor: `cypress/browsers-chrome73-ff68`,
              command: `$(npm bin)/cypress run --spec cypress/integration/${value} --browser firefox`,
              group: `chrome`,
              browser: `chrome`
            },
          },
